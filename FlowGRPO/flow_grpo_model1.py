...
MODEL_SAVE_PATH = "flow_matching_model1.pth"  # 模型保存路径
...
class FlowMatchingMLP(nn.Module):
    ...
    def __init__(self, hidden_dim=128, num_hidden_layers=2, norm_params=None):
        ...
        layers = [
            ...
            nn.ReLU(),
            nn.Dropout(0.1)  # 降低Dropout率，从0.1→0.05
        ]
        # 添加隐藏层
...
def visualize_model(model):
    ...
    ax4.scatter([0, 0], [-2, 2], c='black', s=150, marker='*', label='True x0 ∈ {-2,2}')
    ax4.set_xlabel('t')
    ax4.set_ylabel('x_t')
    ...
    ax5.axvline(-2, c='k', ls='--', alpha=0.5)
    ax5.axvline(2, c='k', ls='--', alpha=0.5)
